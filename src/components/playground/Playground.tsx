...
import { MODELS, SAMPLE_PROMPTS, DEFAULT_SYSTEM_PROMPT } from '../../services/models'
...
export function Playground() {
  ...
  const lastRequestRef = useRef<{ prompt: string; model: string; systemPrompt: string } | null>(null)

  const { streaming, output, error, metrics, stream, abort, reset } = useStreaming()

  const handleRun = useCallback(async () => {
    if (!prompt.trim() || streaming) return
    const req = { prompt: prompt.trim(), model, systemPrompt }
    lastRequestRef.current = req
    await stream(req.prompt, { model: req.model, systemPrompt: req.systemPrompt })
  ...
  return (
    <section aria-labelledby="playground-heading" className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <h2 id="playground-heading" className="text-sm font-medium text-neutral-200">
      ...
      {inputMode === 'text' ? (
        ...
        <AudioInput onTranscript={t => setPrompt(p => p ? `${p}\n${t}` : t)} />
      )}
